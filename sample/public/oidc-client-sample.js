...
var settings = {
    ...
    client_id: 'im_client',
    redirect_uri: 'http://'+(window.location.host)+'/oidc-client-sample.html',
    post_logout_redirect_uri: 'http://'+(window.location.host)+'/oidc-client-sample.html',
...
function signinDifferentCallback(){
    client.createSigninRequest({ state: { bar: 15 }, redirect_uri: 'http://'+(window.location.host)+'/oidc-client-sample-callback.html' }).then(function(req) {
        log("signin request", req, "<a href='" + req.url + "'>go signin</a>");
        if (followLinks()) {
            window.location = req.url;
        }

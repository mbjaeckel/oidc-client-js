...
var settings = {
    authority: 'https://pingfed.eagleinvsys.com:9031',
    client_id: 'im_client',
    redirect_uri: 'http://'+(window.location.host)+'/user-manager-sample.html',
    post_logout_redirect_uri: 'http://'+(window.location.host)+'/user-manager-sample.html',
    response_type: 'id_token token',
    scope: 'openid email',
    
    popup_redirect_uri:'http://'+(window.location.host)+'/user-manager-sample-popup-signin.html',
    popup_post_logout_redirect_uri:'http://'+(window.location.host)+'/user-manager-sample-popup-signout.html',
    
    silent_redirect_uri:'http://'+(window.location.host)+'/user-manager-sample-silent.html',
...
function startSigninMainWindowDiffCallbackPage() {
    mgr.signinRedirect({state:'some data', redirect_uri: 'http://'+(window.location.host)+'/user-manager-sample-callback.html'}).then(function() {
        log("signinRedirect done");
    }).catch(function(err) {
        log(err);
    });
